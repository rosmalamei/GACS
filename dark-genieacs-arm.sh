...
apply_darkmode_assets() {
    ...
    if [ -d "${SCRIPT_DIR%/}/public" ] && [ -d "$installed_genieacs_dir/public" ]; then
        backup_dir="${installed_genieacs_dir%/}/public.bak.$(date +%Y%m%d%H%M%S)"
        cp -a "${installed_genieacs_dir%/}//GACS/public" "$backup_dir"
        cp -a "${SCRIPT_DIR%/}/GACS/public/." "${installed_genieacs_dir%/}/public/"
        return 0
    fi
